...
- (void)setTopicEntity:(BSTopicInfo *)topicEntity {
    _topicEntity = topicEntity;
    
    [self.contentView addSubview:self.baseView];
    
    NSURL * imageURL = [NSURL URLWithString:self.topicEntity.imageurl];
    [_avatarImageView sd_setImageWithURL:imageURL placeholderImage:[UIImage imageNamed:@"avatar_placeholder"]];
...
//    _topicInfoLabel.text = [NSString stringWithFormat:@"%@ • 最后由 %@ • %@", _topicEntity.category.categoryName, _topicEntity.lastReplyUser.username, [_topicEntity.updatedAt timeAgoSinceNow]];
    _topicInfoLabel.text = [NSString stringWithFormat:@"时间:%@", _topicEntity.updatimeStr];
