...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [self setBackgroundColor:[UIColor colorWithHexString:@"#1A69B1"]];
        _waveAmplitude=3.0;
        _waveCycle=1.0;
        increase=NO;
        _waterColor = [UIColor colorWithHexString:@"#1B70BF"];
        _waterLineY=10.0;
...
//    NSMutableAttributedString *attriButedText=[self formatBatteryLevel:10];
...
//    CGPoint textPoint = CGPointMake(width/2-textSize.size.width/2, 70);
//    [attriButedText drawAtPoint:textPoint];
    ...
    //后波浪绘制
    CGContextSetFillColorWithColor(context, [[UIColor colorWithHexString:@"#195B99"] CGColor]);
    CGPathAddLineToPoint(backPath, nil, width, rect.size.height);
    ...
    //后波浪反色绘制
    CGPathAddLineToPoint(backReversePath, nil, width, rect.size.height);
...
//    [attriButedText drawAtPoint:textPoint];
    ...
    CGPathAddLineToPoint(frontPath, nil, width, rect.size.height);
    ...
    //前波浪反色绘制
    CGPathAddLineToPoint(frontReversePath, nil, width, rect.size.height);
