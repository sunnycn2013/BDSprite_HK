...
@interface WiKiListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"牛币";
    self.pageIndex = 1;
    self.loading = NO;
    
    self.tableView.mj_header = self.refreashHeader;
    self.tableView.mj_footer = self.refreashFooter;
    self.tableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, KScreenHeight);
...
- (void)fetchFirstPageDataSource{
    ...
    [SVProgressHUD show];
    [self.coinViewModel fetchFirstPageCoinInfo:1 completionHandler:^(id data, NSError *error) {
        self.loading = NO;
...
- (void)fetchDataSourceAtPage:(NSInteger)atPage {
    __weak typeof(self) weakSelf = self;
    self.tableView.hidden = YES;
    [self.coinViewModel fetchCoinInfoPage:atPage completionHandler:^(id data, NSError *error) {
        self.loading = NO;
