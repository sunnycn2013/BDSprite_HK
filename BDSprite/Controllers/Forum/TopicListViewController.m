...
//        [[BSTopicModel Instance] getNewestTopicList:callback atPage:atPage];
        self.navigationItem.title = @"最新帖子";
    } else if (self.topicListType == TopicListTypeHots) {
//        [[BSTopicModel Instance] getHotsTopicList:callback atPage:atPage];
        self.navigationItem.title = @"最热帖子";
    } else if (self.topicListType == TopicListTypeNoReply) {
//        [[BSTopicModel Instance] getNoReplyTopicList:callback atPage:atPage];
        self.navigationItem.title = @"冷门帖子";
    } else if (self.topicListType == TopicListTypeJob) {
//        [[BSTopicModel Instance] getJobTopicList:callback atPage:atPage];
        self.navigationItem.title = @"招聘帖子";
    } else if (self.topicListType == TopicListTypeVoted && self.userId > 0) {
//        [[BSTopicModel Instance] getVotedTopicListByUser:self.userId callback:callback atPage:atPage];
        self.navigationItem.title = @"赞过的帖子";
    } else if (self.topicListType == TopicListTypeNormal && self.userId > 0) {
//        [[BSTopicModel Instance] getTopicListByUser:self.userId callback:callback atPage:atPage];
